...
func main() {
	...
	// sets up a server for handling document addition and query requests
	...
	wvStore, err := weaviate.New(
		//  Uses the previously created embedder.
		weaviate.WithEmbedder(emb),

		// Sets the scheme to HTTP
		weaviate.WithScheme("http"),

		// set up the host to localhost with a port
		weaviate.WithHost("localhost:"+cmp.Or(os.Getenv("WVPORT"), "9035")),

		// Sets the index name to "Document"
		weaviate.WithIndexName("Document"),
	)

	//  creates a new ragServer instance with the context, Weaviate store, and Gemini client.
	server := &ragServer{
		ctx:          ctx,
		wvStore:      wvStore,
	...
	// for handling POST requests to /add/.
	mux.HandleFunc("POST /add/", server.addDocumentsHandler)

	// for handling POST requests to /query/
...
func (rs *ragServer) addDocumentsHandler(w http.ResponseWriter, req *http.Request) {
	...
	// processes HTTP requests to add documents to a Weaviate store
	...
	for _, doc := range ar.Documents {
		// Converts the documents to Weaviate's schema.Document format.
